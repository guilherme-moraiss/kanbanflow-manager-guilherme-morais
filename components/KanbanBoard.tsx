...
const KanbanBoard: React.FC = () => {
  ...
  const renderColumn = (status: TaskStatus, title: string) => {
    ...
    return (
      <div 
        className="flex-1 min-w-[300px] bg-slate-100/80 dark:bg-slate-800/50 rounded-xl p-4 flex flex-col h-full border border-slate-200 dark:border-slate-700"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto space-y-3 min-h-[200px] pr-1 scrollbar-thin">
            ...
            >
              ...
              <div className="absolute top-3 right-3 flex items-center gap-2">
                 ...
                 <div className={`text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center border shadow-sm ${
                    task.executionOrder === 1 ? 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 border-red-100 dark:border-red-800' :
                    task.executionOrder === 2 ? 'bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 border-orange-100 dark:border-orange-800' :
                    'bg-slate-50 dark:bg-slate-700 text-slate-400 dark:text-slate-500 border-slate-100 dark:border-slate-600'
                 ...
                 {user?.role === UserRole.MANAGER && (
                    <button 
                        ...
                        className="text-slate-300 dark:text-slate-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded p-0.5 opacity-0 group-hover:opacity-100 transition-all"
                        title="Delete Task"
                    >
              ...
              <h4 className="font-semibold text-slate-800 dark:text-white mb-2 pr-12 leading-tight">
                  {task.title}
              </h4>

              <div className="flex items-center gap-2 mb-2">
                {isTaskLate(task) && (
                  <div className="flex items-center gap-1.5 text-xs font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 px-2 py-1 rounded-md">
                    <Clock className="w-3 h-3" />
                    <span>{getDaysLate(task)}d late</span>
                  </div>
                )}
                {task.status === TaskStatus.DOING && task.realStartDate && (
                  <div className="flex items-center gap-1.5 text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2 py-1 rounded-md">
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-50 dark:border-slate-700">
                 <div className="flex items-center gap-2" title={task.developerName}>
                    {task.developerAvatar ? (
                        ...
                        <div className="w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-500">
                 ...
                 <div className="flex items-center gap-2">
                     {task.realStartDate && status === TaskStatus.DOING && (
                         <span className="text-[10px] text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded border border-blue-100 dark:border-blue-800" title="Started">
                            {formatDate(task.realStartDate)}
                         </span>
                     )}
                     <span className="text-xs font-mono font-semibold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-1 rounded border border-slate-200 dark:border-slate-600" title="Story Points">
          ...
          {columnTasks.length === 0 && (
             <div className="h-24 border-2 border-dashed border-slate-200 dark:border-slate-600 rounded-lg flex items-center justify-center text-slate-400 dark:text-slate-500 text-sm">
                Drop here
             </div>
          )}
        </div>
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
         <div>
            <h2 className="text-lg font-bold text-slate-800 dark:text-white">Kanban Board</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Drag cards to update status</p>
         </div>
         {user?.role === UserRole.MANAGER && (
            <Button onClick={() => setIsModalOpen(true)} className="shadow-indigo-500/20">
      ...
      {error && (
        <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg border border-red-100 dark:border-red-800 flex items-center gap-2">
      ...
      {successMessage && (
        <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg border border-green-100 dark:border-green-800 flex items-center gap-2">
          <AlertCircle className="w-5 h-5" />
          {successMessage}
        </div>
      )}
      ...
      <div className="grid grid-cols-5 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <div className="text-2xl font-bold text-slate-600 dark:text-slate-300">{tasks.filter(t => t.status === TaskStatus.TODO).length}</div>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">In Progress</div>
          <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{tasks.filter(t => t.status === TaskStatus.DOING).length}</div>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Completed</div>
          <div className="text-2xl font-bold text-green-600 dark:text-green-400">{tasks.filter(t => t.status === TaskStatus.DONE).length}</div>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Story Points</div>
          <div className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{tasks.reduce((sum, t) => sum + (t.storyPoints || 0), 0)}</div>
        </div>
      </div>

      <div className="bg-white dark:bg-slate-800 p-5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm mb-6">
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm mb-6 flex gap-4 items-center flex-wrap">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-700 dark:text-white bg-white dark:bg-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none"
          />
        </div>

        <select 
          className="px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm text-slate-700 dark:text-white bg-white dark:bg-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none"
          value={filterDev}
        ...
        {(filterDev !== 'all' || filterType !== 'all' || searchQuery.trim()) && (
          <button
            ...
            className="px-3 py-2 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white underline"
          >
            Clear All
          </button>
        )}
