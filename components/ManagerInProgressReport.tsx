...
const ManagerInProgressReport: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Tasks In Progress</h2>
        <p className="text-sm text-slate-500 mt-1">Monitor ongoing tasks and deadlines</p>
      </div>

      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
          <div className="text-xs text-slate-500 font-medium mb-1">Total Tasks</div>
          <div className="text-2xl font-bold text-slate-800">{inProgressTasks.length}</div>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="text-2xl font-bold text-red-600">{lateTasks.length}</div>
        </div>
      </div>
      ...
      {inProgressTasks.length === 0 ? (
        <div className="p-8 text-center bg-white rounded-xl shadow-sm border border-slate-200">
          ...
          <p className="text-lg font-medium text-slate-700">No tasks in progress</p>
          <p className="text-sm text-slate-500 mt-2">All tasks are completed!</p>
        </div>
      ) : (
                ...
                {inProgressTasks.map((task) => {
                  ...
                  return (
                    <tr key={task.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-700">{task.developerName || 'Unassigned'}</td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-center">
                        {taskIsLate ? (
                          <div className="flex items-center justify-center gap-1.5 text-red-600 font-semibold">
